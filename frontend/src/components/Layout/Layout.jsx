...
const Layout = () => {
  ...
  return (
    <div className="min-h-screen bg-background text-on-background font-body">
      ...
      <a
        href="#main-content"
        className="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 focus:z-[200] focus:bg-secondary focus:text-on-secondary focus:px-4 focus:py-2 focus:rounded-lg focus:font-bold"
      >
      ...
      <header
        role="banner"
        className="fixed top-0 w-full flex justify-between items-center px-8 py-4 max-w-[1440px] left-1/2 -translate-x-1/2 bg-[#1b3656]/80 backdrop-blur-[24px] z-50 border-b border-white/5 rounded-b-[2rem] shadow-xl"
      >
        <Link to="/" aria-label="ElectionIQ – Home" className="flex items-center gap-2">
          <span className="material-symbols-outlined text-secondary text-2xl" aria-hidden="true">insights</span>
          <h1 className="text-2xl font-black text-secondary tracking-tighter font-headline">ElectionIQ</h1>
        </Link>

        <div className="flex items-center gap-8 ml-auto">
          <nav aria-label="Primary navigation" className="hidden xl:flex gap-6 items-center">
      ...
      {isMobileMenuOpen && (
        <div
          ...
          className="fixed inset-0 top-[72px] bg-[#0a192f]/95 backdrop-blur-3xl z-40 md:hidden border-t border-white/10"
        >
          <nav aria-label="Mobile navigation" className="flex flex-col p-8 gap-6">
            ...
            <div className="h-px bg-white/10 my-4" aria-hidden="true"></div>
      ...
      <aside aria-label="Civic duty reminder" className="max-w-[1440px] mx-auto px-6 mt-20">
        <div className="bg-secondary/10 border border-secondary/20 rounded-[2rem] p-8 md:p-10 text-center shadow-[0_0_40px_rgba(240,193,44,0.1)] relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-secondary/5 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2" aria-hidden="true"></div>
          <div className="absolute bottom-0 left-0 w-64 h-64 bg-primary/5 rounded-full blur-3xl translate-y-1/2 -translate-x-1/2" aria-hidden="true"></div>
          <span className="material-symbols-outlined text-secondary text-5xl mb-4 relative z-10" aria-hidden="true">how_to_vote</span>
          <h2 className="text-2xl md:text-3xl font-headline font-black text-secondary mb-4 relative z-10">Vote Dena Hamari Duty Hai</h2>
      ...
      <footer
        ...
        className="bg-surface-container-low rounded-t-[3rem] mt-20 w-full py-12 px-12 grid grid-cols-1 md:grid-cols-2 gap-8 items-center border-t border-white/5 max-w-[1440px] mx-auto"
      >
        <div className="space-y-6">
          ...
          <div className="mt-4 inline-block relative w-full max-w-xs">
            <label htmlFor="state-results-select" className="sr-only">Live State Election Results 2024</label>
            <select
              id="state-results-select"
              aria-label="Select a state to view live election results"
              className="w-full appearance-none bg-surface-variant/40 border border-white/5 text-secondary font-headline text-sm rounded-lg px-4 py-2 outline-none focus:border-secondary/50 cursor-pointer"
            >
              <option value="" disabled className="bg-[#0a192f]">Live State Results 2024</option>
            ...
            <span className="material-symbols-outlined absolute right-3 top-1/2 -translate-y-1/2 text-secondary/50 pointer-events-none text-sm" aria-hidden="true">poll</span>
          </div>
        </div>
        <nav aria-label="Footer navigation" className="flex flex-wrap gap-x-8 gap-y-4 md:justify-end items-start h-full pt-2">
